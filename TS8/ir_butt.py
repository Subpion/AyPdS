...
#hacer la ecg CON ruido tambien, esta esta modulada en amplitud
#por una onda de MENOR frecuencias

#hacemos un filtro de pasa banda que mate frecuencia 0.1 0.2 0.3hz a 

#--------plantilla de diseno-----------

wp= [0.8, 30]#frecuancia de corte/paso hz
ws= [0.1 , 35]  #frecuencia de stop/detenida hz
...
f_aprox = "butter"
mi_sos_butt=signal.iirdesign(wp=wp,ws=ws,gpass=alpha_p,
                     gstop=alpha_s,
                     analog=False,
                     ftype = f_aprox,
                     output= "sos",fs=fs)
...
ecg_filtrado_butter=signal.sosfilt(mi_sos_butt
                                   , ecg_one_lead)
...
plt.plot(ecg_filtrado_butter,label = "butt")
